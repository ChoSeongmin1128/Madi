...
- 현재 리브랜딩 1차 배포는 Madi 저장소와 iCloud 컨테이너를 기본값으로 쓰되, 기존 앱 데이터를 보호하기 위해 legacy 로컬 DB를 첫 실행 시 가져오고 기존 iCloud 컨테이너를 migration 계층에서 dual read/write로 다룹니다.
...
- 1차 migration release: 앱/패키지/산출물은 Madi로 배포하고, 기존 로컬 DB와 legacy iCloud 컨테이너는 migration 계층에서만 다룹니다. 이 버전은 Madi iCloud를 primary로 쓰면서 late device 변경 회수를 위해 legacy iCloud에도 씁니다.
- 2차 cleanup-lite release: Madi는 legacy iCloud에 더 이상 쓰지 않고, late device 변경을 읽어 Madi 쪽으로 가져오는 read-only import 경로만 남깁니다. 업데이트하지 않은 기존 바이너리를 원격으로 강제 차단하지는 못합니다.
- 3차 full cleanup release: legacy 로컬 DB import, legacy iCloud read path, legacy allowlist를 제거하고 Madi 식별자만 남깁니다.
