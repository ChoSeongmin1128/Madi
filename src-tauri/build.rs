...
fn main() {
    if let Ok(target) = env::var("TARGET") {
        println!("cargo:rustc-env=MADI_TARGET_TRIPLE={target}");
    }
...
fn build_cloudkit_sidecar() {
    ...
    if target != "aarch64-apple-darwin" {
        panic!(
            "unsupported macOS target for Madi: {target}. Apple Silicon macOS only is supported."
        );
    }
    ...
    let built_binary = find_sidecar_binary(&package_dir.join(".build"), "madi-cloudkit-bridge")
    ...
    let destination = binaries_dir.join(format!("madi-cloudkit-bridge-{target}"));
...
fn create_helper_app_bundle(binaries_dir: &Path, built_binary: &Path, package_manifest: &Path) {
    let helper_app_dir = binaries_dir.join("madi-cloudkit-bridge.app");
    ...
    let helper_executable = macos_dir.join("madi-cloudkit-bridge");
    ...
    #[cfg(unix)]
    {
        ...
        let mut permissions = fs::metadata(&helper_executable)
            ...
            .permissions();
        permissions.set_mode(0o755);
        fs::set_permissions(&helper_executable, permissions)
            .expect("helper app executable permissions");
    }
}
...
<dict>
  ...
  <string>madi-cloudkit-bridge</string>
  <key>CFBundleIdentifier</key>
  <string>com.seongmin.madi</string>
  <key>CFBundleInfoDictionaryVersion</key>
  <string>6.0</string>
  <key>CFBundleName</key>
  <string>MadiCloudKitBridge</string>
