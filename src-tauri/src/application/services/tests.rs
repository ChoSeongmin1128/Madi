...
#[test]
fn resolve_current_document_id_falls_back_to_first_when_preferred_is_missing() {
  let repository = MockRepository::new(BlockKind::Markdown);

  let current_document_id = resolve_current_document_id(
    &repository.document_summaries,
    Some("missing-doc".to_string()),
  );

  assert_eq!(current_document_id.as_deref(), Some("doc-1"));
}
